...
func run(version, build string) error {
	hostname, port, _, config := server.ReadConfig("CR", func() any {
		return &crypter.AppConfig{} // use the correct object to deserialize the configuration
	})
	var appCfg = config.(*crypter.AppConfig)

	addr := fmt.Sprintf("%s:%d", hostname, port)

	logger := setupLog(*appCfg)
	defer logger.Close()

	var (
		service       = crypter.NewService(logger, appCfg.TokenSecurity)
	...
	go func() {
		server.PrintServerBanner("crypter", version, build, string(appCfg.Environment), addr)
