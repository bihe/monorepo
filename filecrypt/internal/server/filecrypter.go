...
func NewFileCrypter(basePath string, logger *log.Entry, tokenSetting internal.TokenSecurity) *FileCrypter {
...
func (f *FileCrypter) Encrypt(ctxt context.Context, req *proto.CrypterRequest) (resp *proto.CrypterResponse, err error) {
	...
	if err != nil {
		commons.LogWith(f.logger, "server.Encrypt").Debugf("validation errors: %v", err)
		return nil, err
	}
	user, err := f.jwtAuth.EvaluateToken(req.AuthToken)
	if err != nil {
		commons.LogWith(f.logger, "server.Encrypt").Warnf("error during token evaluation: %v", err)
		return nil, err
	}
	commons.LogWith(f.logger, "server.Encrypt").Debugf("user '%s' requests encryption of payload", user.UserID)
