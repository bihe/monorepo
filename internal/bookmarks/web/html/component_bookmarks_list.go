			...
			g.Map(items, func(b bookmarks.Bookmark) g.Node {
				return h.Div(h.Class("bookmark_item hstack gap-3"),
					h.Div(h.Class("p2"),
						...
						g.Text(" "),
						g.If(b.Type == bookmarks.Node, h.A(h.Class("bookmark_name"), h.Href(b.URL), h.Title(b.DisplayName), g.Text(common.Ellipsis(b.DisplayName, ell.NodeLen, "...")))),
						g.If(b.Type == bookmarks.Folder, h.A(h.Class("bookmark_name"), h.Href("/bm/~"+common.EnsureTrailingSlash(b.Path)+b.DisplayName), h.Title(b.DisplayName), g.Text(common.Ellipsis(b.DisplayName, ell.FolderLen, "...")))),
