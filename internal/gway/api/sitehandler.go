...
func (s SitesHandler) MountRoutes() chi.Router {
	r := chi.NewRouter()
	r.Get("/sites", s.handleGetSitesForUser())
	r.Get("/sites/users/{siteName}", s.handleGetUsersForSite())
	r.Post("/sites", s.handleSaveSitesForUser())
	return r
}
