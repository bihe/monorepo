...
func (h *Handler) DeleteDocumentByID(c echo.Context) (err error) {
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.DeleteDocumentByID").Warnf("the document '%s' is not available, %v", id, err)
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.DeleteDocumentByID").Warnf("error during delete operation of '%s', %v", id, err)
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.DeleteDocumentByID").Errorf("could not delete file in backend store '%s', %v", fileName, err)
		err = fmt.Errorf("could not delete '%s', %v", id, err)
		return errors.ServerError{Err: err, Request: c.Request()}
	}
...
func (h *Handler) SearchDocuments(c echo.Context) (err error) {
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.SearchDocuments").Warnf("could not search for documents, %v", err)
...
func (h *Handler) SaveDocument(c echo.Context) (err error) {
	...
	if err = c.Bind(d); err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.SaveDocument").Warnf("could not bind supplied payload, %v", err)
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.SaveDocument").Warnf("could not process the uploaded file, %v", err)
	...
	if d.ID == "" {
		...
		if err != nil {
			logging.LogWithReq(c.Request(), h.log, "documents.SaveDocument").Warnf("cannot find document by ID '%s' - create a new entry, %v", d.ID, err)
			doc = initDocument(d, senderList, tagList)
		} else {
			newDoc = false
			logging.LogWithReq(c.Request(), h.log, "documents.SaveDocument").Infof("will update existing document ID '%s'", d.ID)
			doc.Title = d.Title
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.SaveDocument").Errorf("could not save document: %v", err)
...
func (h *Handler) SearchList(c echo.Context) (err error) {
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.SearchList").Warnf("could not search for tags, %v", err)
...
func (h *Handler) startAtomic(c echo.Context) (persistence.Atomic, error) {
	...
	if err != nil {
		logging.LogWithReq(c.Request(), h.log, "documents.startAtomic").Errorf("failed to start transaction: %v", err)
