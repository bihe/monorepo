			...
			h.Body(g.Attr("data-bs-theme", "dark"),
				h.Header(
					h.Nav(h.Class("navbar navbar-expand-lg navbar-dark fixed-top header"),
						h.Div(h.Class("container-fluid navbar-menu-area"),
							...
							h.Div(h.Class("collapse navbar-collapse"), h.ID("navbarCollapse"),
								...
								g.Raw("&nbsp;"),
								h.Span(h.Class("application_version"), h.Span(h.Class("badge text-bg-warning"), h.I(h.Class("bi bi-git")), g.Textf(" %s-%s", model.TimeStamp, model.Commit))),
								g.Raw("&nbsp;"),
								h.Span(h.Class("badge align-items-center p-1 pe-2 text-dark-emphasis bg-light-subtle border border-dark-subtle rounded-pill"), h.Img(h.Class("rounded-circle me-1"), h.Width("24"), h.Height("24"), h.Src(model.User.ProfileURL)),
