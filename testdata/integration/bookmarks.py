...
def validate(baseURL: str, page: Page):
    ...
    expect(page.get_by_role('link',
                            name=prefix+'__BOOKMARK-UPDATE')).to_be_visible()
    expect(page.locator(class_trigger_menu)).to_be_visible()
    ...
    expect(page.locator('.bookmark_item').filter(
        has_text=prefix+'__FOLDER')).to_be_visible()

    page.locator('.bookmark_item').filter(
        has_text=prefix+'__FOLDER').locator(class_trigger_menu).click()
    page.locator('.bookmark_item').filter(
        has_text=prefix+'__FOLDER').locator("#btn-bookmark-delete").click()
    page.locator('id=btn-confirm').click()
    page.wait_for_timeout(500)
